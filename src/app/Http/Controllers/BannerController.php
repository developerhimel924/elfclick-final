...
class BannerController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        {
            Session::flash('error', 'You can not create banner more then 13');
            return back();
        }
    ...
    public function update(Banner $banner,Request $request)
    {
        ...
        if ($img_tmp) /* you can update with image */
        {
            $img_exten = $img_tmp->getClientOriginalExtension();
            $img_name = $img_tmp->getClientOriginalName().'.'.$img_exten;
            $img_path = 'Back/images/banner';
            $img_tmp->move($img_path,$img_name);
            $old_img = 'Back/images/partner/'.$banner->image;
    ...
    public function destroy($id)
    {
        ...
        $delbanner = 'Back/images/banner/'.$banner['image'];
        if (file_exists($delbanner)) {
            @unlink($delbanner);
        }
        $banner->delete();
