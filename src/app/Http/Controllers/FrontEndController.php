...
class FrontEndController extends Controller
{
    public function home()
    {
        ...
        $data['bannersRecommanded'] = Banner::where('status',1)->limit(6)->get();
    ...
    public function todaysDealProducts(Request $request)
    {
        $product = DB::table('todaysdeal')->where('slug', $request->slug)->first();
    ...
    public function rangeSelect(Request $request)
    {
        ...
        $products = Product::where('category_id',$category->id)->where('product_price', '<=', $request->l_price)->where('product_price', '>=', $request->f_price)->get();
        return view('FrontEnd.product.catePro',compact('products', 'category'));
    }
    ...
    {
        $products = DB::table('todaysdeal')->orderBy('id','desc')->get();
        /*dd($products);*/
        return view('FrontEnd.product.contentPro',compact('products'));
    }
